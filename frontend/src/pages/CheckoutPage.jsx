...
export const CheckoutPage = () => {
    ...
    const validateForm = () => {
        ...
        if ((paymentMethod === 'QR' || paymentMethod === 'UPI') && !utr.trim()) {
            toast.error('Please enter your 12-digit UPI Transaction ID (UTR)');
            return false;
        }
        if ((paymentMethod === 'QR' || paymentMethod === 'UPI') && utr.trim().length !== 12) {
                    ...
                    <div className="lg:col-span-2 space-y-8">
                        <form onSubmit={handleSubmit} className="space-y-8">
                            ...
                            <motion.button
                                ...
                                type="submit"
                                disabled={loading}
                                className="w-full bg-slate-900 hover:bg-slate-800 text-white rounded-2xl py-5 text-lg font-bold tracking-wider shadow-xl transition-all uppercase flex items-center justify-center gap-3"
                            >
                                {loading ? (
